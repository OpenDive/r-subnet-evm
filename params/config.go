...
func (c *ChainConfig) checkCompatible(newcfg *ChainConfig, headHeight *big.Int, headTimestamp *big.Int) *ConfigCompatError {
	...
	if isForkIncompatible(c.RaceCalculatorConfig.Timestamp(), newcfg.RaceCalculatorConfig.Timestamp(), headTimestamp) {
		return newCompatError("RaceCalculator fork block timestamp", c.RaceCalculatorConfig.Timestamp(), newcfg.RaceCalculatorConfig.Timestamp())
	}
...
func (c *ChainConfig) enabledStatefulPrecompiles() []precompile.StatefulPrecompileConfig {
	...
	if c.RaceCalculatorConfig.Timestamp() != nil {
		statefulPrecompileConfigs = append(statefulPrecompileConfigs, &c.RaceCalculatorConfig)
	}
